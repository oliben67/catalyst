...
[`rules/Rules-of-Rules.md`](rules/Rules-of-Rules.md): that file governs
the rules themselves; this file governs the work items that reference
those rules.
...
from `domains/` — not free text. (Feature entries under `features/` are
not development artifacts under this document and carry no `Domain`
field — see `Rules-of-Rules.md` §9.)
