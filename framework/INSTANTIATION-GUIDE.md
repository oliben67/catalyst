...
7. Deploy `framework/definitions/` (`INVARIANTS.md` INV-23) —
   ...
   `framework/definitions/` are framework-source structure
   only). Also copy `framework/definitions/README.md` to
...
8. Create a root-level `README.md` in the deployed framework directory that
   ...
   self-documenting. Also copy `framework/ACCESS-CONTROL.md`
...
6. Add a one-line header to each of your project's rule and process files
   ...
   ```
   > Instantiates [the catalyst framework rules template](../../framework/rules-of-rules.template.md).
   ```
